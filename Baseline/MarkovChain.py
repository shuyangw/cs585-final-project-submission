...
class markov(object):
    ...
    def generate_markov_text(self, start, size=25):
        ...
        while i <= size:
            ...
            i += 1
        print start + ' ' + ' '.join(gen_words).replace(' .', '.').replace(' ,', ',').replace(" '", "'")

#doc = ['This is your first text', 'and here is your second text.']
# For better results, use large corpora
path_to_file = "./Lol75.txt"
text = io.open(path_to_file, mode="r", encoding="utf-8").read()

comments = list(filter(lambda a: a != '', text.split("\n")))


mark = markov(comments, 2, 2)
mark.generate_markov_text('I think', size=25)